...
LABEL org.opencontainers.image.title="Typhon" \
      org.opencontainers.image.description="Alpine based Tor Proxy Anonymizer" \
      org.opencontainers.image.created=$IMAGE_CREATE_DATE \
      org.opencontainers.image.version=$IMAGE_VERSION \
      org.opencontainers.image.authors="Anthony Angelo" \
      org.opencontainers.image.url="https://github.com/pangealab/typhon/" \
      org.opencontainers.image.documentation="https://github.com/pangealab/typhon/README.md" \
      org.opencontainers.image.vendor="Anthony Angelo" \
      org.opencontainers.image.licenses="MIT" \
      org.opencontainers.image.source="https://github.com/pangealab/typhon.git" \
      org.opencontainers.image.revision=$IMAGE_SOURCE_REVISION

# Install tor and privoxy
RUN apk --no-cache --no-progress upgrade &&\
    apk --no-cache --no-progress add bash curl privoxy shadow tini tor tzdata &&\
    for f in /etc/privoxy/*.new; do mv $f "${f%.*}"; done &&\
    find /etc/privoxy -type f -exec chown privoxy {} \; &&\
    find /etc/privoxy -type f -exec chmod 668 {} \; &&\
